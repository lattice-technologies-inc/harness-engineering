...
Use the `/breadboarding` command to map existing systems or detail a shape into concrete affordances. Breadboarding produces:
...
Use the `/breadboarding` command to produce:
...
After a shape is breadboarded, slice it into vertical implementation increments. Use the `/breadboarding` command for the slicing process — it defines what vertical slices are, the procedure for creating them, and visualization formats.
